...
const New_User_Welfare = React.createClass({
  ...
  newPayMoney(){
    ...
    this.setState({
      ...
      btnPayNewNotPut:"giveUp pay-loading"
    })
    ...
    //if(userType == "new" || userType == "member"){
      Tools.ajax({
            ...
            success: function (response, xml) {
                  ...
                  _this.setState({
                    saleCards : cardInfo.data.saleCards,
  ...
  renderCards(){
    ...
    if(saleCards !== "" && saleCards.length > 0){
        return saleCards.map((item, index) => {
          ...
          return (
            <Link to={{pathname:"CardDetail_Buy",query:{cardId:item.cardId,payReduce:payReduce,payTrue:userPayMoney,itemPhoto:item.photo,support:item.supportCount,about:item.about}}} key={index}>
